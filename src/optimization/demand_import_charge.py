...
class DemandImportCharge(Service):
    ...
    def add_max_demand_constraint(self) -> None:
        for interval in self.service_params.intervals:
            if (
                (interval.interval_end.time() <= self.service_params.demand_charge_period_end)
                and
                ((interval.interval_end - interval.interval_duration).time() >= self.service_params.demand_charge_period_start)
            ):
